...
import { 
  ShieldAlert, RotateCcw, AlertTriangle, X, Plus, Minus,
  Pause, Play, Trash2, Edit2, Check, GripVertical, Smartphone,
  Layout, ArrowDownToLine, Volume2, Volume1, VolumeX,
  Activity, BellRing, Music, ChevronDown, ChevronUp, Settings, Clock, Eye, EyeOff, Radio, Globe, Timer
} from 'lucide-react';
...
const dict = {
  en: {
    ...
    moving: "Moving",
    systemNotifs: "Notification System",
    enablePush: "1. Enable Web Push (FCM)",
    enablePushDesc: "Notifications with SCREEN OFF.",
    deviceLinked: "✓ Device linked.",
    testLocal: "Test Local API",
    testLocalDesc: "Basic browser alert.",
    testSw: "Test Service Worker",
    testSwDesc: "Robust system tray notification.",
    testDelayed: "Delayed Test (5s)",
    testDelayedDesc: "Press, turn off screen, and wait.",
    ...
    next: "NEXT",
    offlineSys: "OFFLINE SYSTEM",
    fcmRequired: "Firebase connection required.",
    success: "SUCCESS!",
    pushLinked: "Device linked for background notifications.",
    permDenied: "PERMISSION DENIED",
    permDeniedDesc: "You must grant notification permissions in your browser.",
  ...
  es: {
    ...
    moving: "Moviendo",
    systemNotifs: "Sistema de Notificaciones",
    enablePush: "1. Activar Web Push (FCM)",
    enablePushDesc: "Notificaciones con PANTALLA APAGADA.",
    deviceLinked: "✓ Dispositivo enlazado.",
    testLocal: "Test API Local",
    testLocalDesc: "Alerta básica del navegador.",
    testSw: "Test Service Worker",
    testSwDesc: "Llega a la barra de notificaciones.",
    testDelayed: "Test con Retraso (5s)",
    testDelayedDesc: "Presiona, apaga la pantalla y espera.",
    ...
    next: "SIGUIENTE",
    offlineSys: "SISTEMA OFFLINE",
    fcmRequired: "Conexión a Firebase requerida.",
    success: "¡ÉXITO!",
    pushLinked: "Dispositivo enlazado para notificaciones en segundo plano.",
    permDenied: "PERMISO DENEGADO",
    permDeniedDesc: "Debes otorgar permisos de notificación en tu navegador.",
    ...
    soundMuted: "Silenciado (Solo Notif.)",
    typeUrgent: "Urgente",
    typeCalm: "Calma",
...
const App = () => {
      ...
      try {
          ...
          const playSequence = () => {
              ...
              switch(profile) {
                  ...
                  case 'pulse': scheduleNote('sine', 150, 0, 0.8, 0.1, 0.4); scheduleNote('sine', 150, 0.5, 0.6, 0.1, 0.4); break;
  ...
  const enablePushNotifications = async () => {
    if (isOfflineMode || !messagingInstance) {
        setAlertQueue(prev => [...prev, { title: t('offlineSys'), body: t('fcmRequired'), type: "task" }]);
        return;
    }
    try {
      const permission = await Notification.requestPermission();
      setNotifPermissionGranted(permission === 'granted');
      ...
      if (permission === 'granted') {
        try {
            ...
            if (currentToken) {
                ...
                if (syncRef.current) syncRef.current({ fcmToken: currentToken, pushEnabled: true });
                setAlertQueue(prev => [...prev, { title: t('success'), body: t('pushLinked'), type: "task" }]);
            }
        } catch (tokenErr) {
            setAlertQueue(prev => [...prev, { title: "ERROR DE TOKEN", body: tokenErr.message, type: "task" }]);
        }
      } else {
        setAlertQueue(prev => [...prev, { title: t('permDenied'), body: t('permDeniedDesc'), type: "task" }]);
      }
    } catch (err) {}
  };
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 p-4 pb-32 overflow-x-hidden font-sans">
      ...
      {showNotifLab && (
        <div className="fixed inset-0 z-[400] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md overflow-y-auto">
            <div className="bg-zinc-900 border border-blue-500 w-full max-w-sm p-6 rounded-[32px] shadow-[0_0_50px_rgba(59,130,246,0.2)] relative animate-in zoom-in duration-300 my-auto">
               ...
               <div className="flex items-center gap-3 mb-6"><Activity className="text-blue-500" size={28} /><h2 className="text-xl font-black text-white uppercase leading-tight">{t('systemNotifs')}</h2></div>
               <div className="space-y-3">
                   <button onClick={enablePushNotifications} className="w-full bg-blue-900/20 border border-blue-500 hover:bg-blue-600 py-3 rounded-xl flex items-center justify-between px-4 transition-colors">
                       <div className="text-left">
                           <span className="block text-white font-bold text-[11px] uppercase">{t('enablePush')}</span>
                           <span className="block text-blue-300 text-[9px]">{t('enablePushDesc')}</span>
                           {pushToken && <span className="block text-green-400 text-[8px] mt-1">{t('deviceLinked')}</span>}
                       </div>
                       <Radio size={16} className="text-white"/>
                   </button>
                   <div className="h-px w-full bg-zinc-800 my-2"></div>
                   
                   <button onClick={testLocalNotif} className="w-full bg-zinc-800 border border-zinc-700 hover:border-amber-500 py-3 rounded-xl flex items-center justify-between px-4 transition-colors">
                       <div className="text-left"><span className="block text-white font-bold text-[11px] uppercase">{t('testLocal')}</span><span className="block text-zinc-500 text-[9px]">{t('testLocalDesc')}</span></div><Play size={14} className="text-amber-500"/>
                   </button>

                   <button onClick={testSwNotif} className="w-full bg-zinc-800 border border-zinc-700 hover:border-emerald-500 py-3 rounded-xl flex items-center justify-between px-4 transition-colors">
                       <div className="text-left"><span className="block text-white font-bold text-[11px] uppercase">{t('testSw')}</span><span className="block text-zinc-500 text-[9px]">{t('testSwDesc')}</span></div><Activity size={14} className="text-emerald-500"/>
                   </button>

                   <button onClick={testDelayedNotif} className="w-full bg-zinc-800 border border-zinc-700 hover:border-purple-500 py-3 rounded-xl flex items-center justify-between px-4 transition-colors relative overflow-hidden group">
                       <div className="absolute inset-0 bg-purple-500/10 translate-x-[-100%] group-active:translate-x-0 transition-transform duration-300"></div>
                       <div className="text-left relative z-10"><span className="block text-white font-bold text-[11px] uppercase">{t('testDelayed')}</span><span className="block text-zinc-500 text-[9px]">{t('testDelayedDesc')}</span></div><Timer size={14} className="text-purple-500 relative z-10"/>
                   </button>
               </div>
            </div>
        </div>
      ...
      <div className="max-w-md mx-auto space-y-4 pt-2">
        <div className="flex items-center justify-between bg-zinc-900/40 p-3 rounded-2xl border border-zinc-800 shadow-lg relative z-[50]">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg xs:text-xl font-black text-amber-500 uppercase leading-none tracking-tighter hidden xs:block">{t('appTitle')}</h1>
          ...
          <div className="flex gap-1">
            <button onClick={toggleWakeLock} className={`p-1.5 rounded-lg border transition-colors ${wakeLockActive ? 'bg-blue-600 text-white border-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`} title="Modo Vigía">{wakeLockActive ? <Eye size={18} /> : <EyeOff size={18} />}</button>
            <button onClick={toggleSoundProfile} className={`p-1.5 rounded-lg border transition-colors ${soundProfile !== 'muted' ? 'bg-amber-600 text-white border-amber-600 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`} title="Sonido">{soundProfile === 'siren' ? <Volume2 size={18}/> : soundProfile === 'radar' ? <Volume1 size={18}/> : <VolumeX size={18}/>}</button>
            <button onClick={() => setShowSoundMenu(true)} className="p-1.5 bg-zinc-800 text-amber-500 border border-zinc-700 rounded-lg shadow-lg hover:bg-zinc-700 transition-colors" title="Ajustes de Sonido"><Music size={18}/></button>
            <button onClick={() => { const next = !vibrateOn; setVibrateOn(next); if(!next) stopInfiniteAlarm(); if(syncRef.current) syncRef.current({ vibrateOn: next }); }} className={`p-1.5 rounded-lg border transition-colors ${vibrateOn ? 'bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`} title="Vibración"><VibrateIcon size={18}/></button>
            
            {/* Botón visual de notificaciones reactivo a permisos */}
            <button onClick={handleNotificationToggle} className={`p-1.5 rounded-lg border transition-colors ${notifPermissionGranted ? 'bg-emerald-600 text-white border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`} title="Notificaciones Push">
                <Smartphone size={18}/>
            </button>
          </div>
        </div>
                      ...
                      if (a.custom) {
                          return (
                              <div key={a.id} className={`flex-[2] flex items-center justify-between rounded-xl border transition-colors ${a.on ? 'bg-amber-600 border-amber-500' : 'bg-zinc-950 border-zinc-800'}`}>
                                  ...
                                  <button onClick={() => toggleAlarm(a.id)} className={`h-full px-2.5 rounded-r-xl border-l flex items-center justify-center transition-colors ${a.on ? 'border-amber-700 hover:bg-amber-700' : 'border-zinc-800 hover:bg-zinc-800'}`}><div className={`w-2 h-2 rounded-full ${a.on ? 'bg-white shadow-[0_0_5px_white]' : 'bg-zinc-700'}`} /></button>
                              </div>
                          );
                      }
        ...
        <div className="grid grid-cols-2 gap-2">
            <button onClick={() => { setShowCronoForm(!showCronoForm); setShowBoxForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-all ${showCronoForm ? 'bg-amber-500 text-black border-amber-500' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}><Plus size={14}/> {t('addCrono')}</button>
            <button onClick={() => { setShowBoxForm(!showBoxForm); setShowCronoForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-all ${showBoxForm ? 'bg-blue-600 text-white border-blue-600' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}><Layout size={14}/> {t('addSection')}</button>
        </div>
                    ...
                    return (
                        <div key={box.id} className={`rounded-xl transition-all relative flex flex-col ${isThisBoxDragged ? 'opacity-30 scale-[0.98] border-dashed border-2 border-amber-500' : 'opacity-100 shadow-lg'}`}>
                                    ...
                                    <div className="px-2 pb-2 pt-0.5 bg-zinc-950 rounded-b-xl border border-t-0 border-zinc-800 flex flex-col pointer-events-none">
                                        {boxTasks.length > 0 ? (
                                            <div className="flex items-center justify-between gap-1 px-2 py-1.5 bg-zinc-900/60 rounded border border-zinc-800/80 shadow-inner">
                                                ...
                                                ) : finishedTasks.length > 0 ? ( <div className="flex-1 flex justify-end pr-2"><Check size={14} className="text-red-500"/></div> ) : null}
                                            </div>
                            ...
                            {!box.isCollapsed && (
                                ...
                                >
                                    {dragState.item?.type === 'task' ? ( <div className={`text-[8px] font-black uppercase flex items-center gap-1 transition-colors pointer-events-none ${isFooterTarget ? 'text-red-500 scale-110' : 'text-zinc-600'}`}><ArrowDownToLine size={10} /> {isFooterTarget ? t('dropToExtract') : t('exitZone')}</div> ) : ( <div className="w-8 h-1 bg-zinc-800/50 rounded-full pointer-events-none" /> )}
                                </div>
                            )}
