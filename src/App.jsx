...
// ==========================================
// PUENTE NATIVO: Creación de Canales y Alarmas
// ==========================================
    ...
    if (window?.Capacitor?.Plugins?.LocalNotifications) {
      try {
        await window.Capacitor.Plugins.LocalNotifications.createChannel({
          id: 'war_alerts',
          name: 'Alertas de Guerra',
          description: 'Notificaciones tácticas en segundo plano',
          importance: 5,
          visibility: 1,
          vibration: true
        });
      } catch(e) { console.error("[NATIVO] Fallo al crear canal:", e); }
    }
  },
...
// TRADUCTOR RESTAURADO PARA EVITAR PANTALLA NEGRA
...
const App = () => {
  ...
  const scheduleAllActiveTimers = useCallback(() => {
    ...
    if (targetEndTimeRef.current && !isNaN(targetEndTimeRef.current.getTime())) {
      ...
      if (warMs > Date.now()) {
        NativeAlarmsBridge.schedule(999999, "Estado de la Guerra", "La guerra ha finalizado.", warMs, warSoundRef.current);
  ...
  const toggleSystemNotifications = async () => {
      if (sysNotifOn) {
          setSysNotifOn(false); if (syncRef.current) syncRef.current({ sysNotifOn: false }); addLog("Notificaciones DESACTIVADAS.", "warning"); 
          NativeAlarmsBridge.cancelAll(); return;
      }
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') { 
          if (wakeLockActive && wakeLockRef.current === null && 'wakeLock' in navigator) navigator.wakeLock.request('screen').then(lock => { wakeLockRef.current = lock; }).catch(()=>{}); 
          NativeAlarmsBridge.cancelAll();
      } 
      else { 
          if (sysNotifOn) scheduleAllActiveTimers(); 
          addLog("Enviando cronos a Android OS", "info"); 
      }
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
    return () => document.removeEventListener('visibilitychange', handleVisibilityChange);
  }, [wakeLockActive, sysNotifOn, scheduleAllActiveTimers, addLog]);
  ...
  const triggerSystemNotification = useCallback(async (title, body) => {
      ...
      try {
          if (window?.Capacitor?.Plugins?.LocalNotifications) {
              await window.Capacitor.Plugins.LocalNotifications.schedule({
                  notifications: [{ id: Date.now(), channelId: 'war_alerts', title: title, body: String(body), schedule: { at: new Date(Date.now() + 1000) } }]
              });
          }
  ...
  useEffect(() => {
      const ticker = setInterval(() => {
          ...
          setWarAlarms(prevAlarms => {
              let newlyTriggeredAlarms = []; let changed = false; let warEndedNow = false;
              ...
              if ((newlyTriggeredAlarms.length > 0 || warEndedNow) && document.visibilityState === 'visible') {
                  ...
                  setAlertQueue(q => [...q, { title: alertTitle, body: notifBody.trim(), type: 'war' }]);
              }
  ...
  const addCrono = async () => {
    ...
    cronoDrafts.forEach((draft, index) => {
        const h = parseInt(draft.h || 0), m = parseInt(draft.m || 0), s = parseInt(draft.s || 0); const total = (h * 3600) + (m * 60) + s;
  ...
  const handleBoxPlayPause = (boxId, forcePause) => { const nl = tasks.map(t => { if (t.boxId === boxId && t.remainingSeconds > 0) { const isRunning = forcePause ? false : true; return { ...t, isRunning, serverEndTime: isRunning ? Date.now() + (t.remainingSeconds * 1000) : null }; } return t; }); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); };
  const executeBoxReset = () => { if (!confirmBoxReset) return; const boxId = confirmBoxReset.id; const nl = tasks.map(t => { if (t.boxId === boxId) { return { ...t, remainingSeconds: t.initialSeconds, serverEndTime: Date.now() + (t.initialSeconds * 1000), isRunning: true, isNewFinish: false, alerted: false }; } return t; }); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); setConfirmBoxReset(null); };
      ...
      >
        ...
        <div className="flex items-center justify-between pointer-events-none mt-1">
          <div className="flex-1">
              ...
              ) : ( <div className={`text-2xl font-mono font-bold leading-none tracking-tighter transition-colors duration-300 ${!t.isRunning && t.remainingSeconds > 0 ? 'text-yellow-400' : (t.remainingSeconds === 0 ? 'text-amber-500' : 'text-white')} ${t.isNewFinish ? 'animate-pulse' : ''}`}>{formatTime(t.remainingSeconds)}</div> )}
          </div>

          <div className="flex items-center gap-1 border-l border-zinc-800 pl-2 shrink-0 pointer-events-auto h-full">
            {editingId === t.id ? (
              <div className="flex gap-1"><button onClick={() => setEditingId(null)} className="p-2 bg-zinc-700/80 rounded-lg text-white transition-colors active:scale-95"><X size={16}/></button><button onClick={() => saveEdit(t.id)} className="p-2 bg-blue-600 rounded-lg text-white transition-colors active:scale-95 shadow-md"><Check size={16}/></button></div>
            ) : (
              <>
                {t.isNewFinish && ( <button onClick={(e) => { e.stopPropagation(); dismissNewFinish(t.id); }} className="p-1 px-2.5 bg-amber-600 text-white rounded-lg font-black text-[9px] uppercase flex items-center gap-1 animate-pulse mr-1 active:scale-95 transition-transform"><RolledScrollIcon size={12}/> Visto</button> )}
                <button onClick={async () => { const nl = tasks.map(x => x.id === t.id ? { ...x, remainingSeconds: x.initialSeconds, serverEndTime: Date.now() + (x.initialSeconds * 1000), isRunning: true, isNewFinish: false, alerted: false } : x); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); }} className={`p-1.5 transition-transform active:scale-95 ${t.isNewFinish ? 'text-amber-300' : 'text-zinc-700'}`}><RotateCcw size={16} /></button>
                <button onClick={() => { const hVal = Math.floor(t.initialSeconds / 3600); const mVal = Math.floor((t.initialSeconds % 3600) / 60); const sVal = t.initialSeconds % 60; setEditBuf({ label: t.label, h: hVal > 0 ? String(hVal) : '', m: mVal > 0 ? String(mVal) : '', s: sVal > 0 ? String(sVal) : '' }); setEditingId(t.id); }} className={`p-1.5 transition-transform active:scale-95 ${t.isNewFinish ? 'text-amber-400' : 'text-zinc-700'}`}><Edit2 size={16} /></button>
                <button onClick={async () => { const nl = tasks.map(x => x.id === t.id ? { ...x, isRunning: !x.isRunning, serverEndTime: !x.isRunning ? Date.now() + (x.remainingSeconds * 1000) : null } : x); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); }} className={`p-1.5 transition-transform active:scale-95 ${!t.isRunning && t.remainingSeconds > 0 ? 'text-yellow-400' : (t.isNewFinish ? 'text-amber-300' : 'text-zinc-600')}`}>{t.isRunning && t.remainingSeconds > 0 ? <Pause size={18} /> : <Play size={18} />}</button>
                <button onClick={async () => { const nt = tasks.filter(x => x.id !== t.id); const nr = rootOrder.filter(item => item.id !== t.id); setTasks(nt); setRootOrder(nr); NativeAlarmsBridge.cancel(t.id); if(syncRef.current) syncRef.current({ tasks: nt, rootOrder: nr }); }} className={`p-1.5 transition-transform active:scale-95 ${t.isNewFinish ? 'text-amber-500' : 'text-zinc-800'}`}><Trash2 size={16} /></button>
              </>
            )}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 p-4 pb-32 overflow-x-hidden font-sans">
      <style>{`
        * { user-select: none; -webkit-user-select: none; -webkit-touch-callout: none; -webkit-tap-highlight-color: transparent !important; }
        *:focus, *:focus-visible, *:active { outline: none !important; box-shadow: none !important; }
        input, textarea { user-select: text; -webkit-user-select: text; -webkit-touch-callout: default; touch-action: auto; outline: none !important; box-shadow: none !important; }
        ...
        <div className="fixed inset-0 z-[600] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md pointer-events-auto">
            <div className="bg-zinc-900 border-2 border-purple-500 w-full max-w-sm rounded-[32px] shadow-[0_0_50px_rgba(168,85,247,0.2)] relative animate-in zoom-in duration-300 flex flex-col max-h-[85vh]">
                <div className="p-5 bg-purple-900/20 border-b border-purple-500/50 flex justify-between items-center shrink-0">
                    ...
                    <button onClick={() => setShowClanModal(false)} className="text-purple-300/60 p-1 transition-transform active:scale-95 rounded-lg"><X size={24}/></button>
                </div>
                        ...
                        <div className="space-y-4">
                            {!clanSetupMode && (
                                <>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <button onClick={() => setClanSetupMode('create')} className="bg-zinc-800 border border-purple-500/50 text-purple-400 p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-transform active:scale-95 shadow-lg">
                                            <Crown size={28}/> <span className="text-[10px] font-black uppercase text-center">Soy Líder<br/>(Fundar)</span>
                                        </button>
                                        <button onClick={() => setClanSetupMode('join')} className="bg-purple-600 text-white p-4 rounded-2xl flex flex-col items-center justify-center gap-2 transition-transform active:scale-95 shadow-[0_0_15px_rgba(168,85,247,0.4)]">
                            ...
                            {clanSetupMode === 'join' && (
                                <div className="space-y-3 animate-in fade-in">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <button onClick={() => setClanSetupMode(null)} className="flex-1 bg-zinc-800 text-zinc-400 py-3 rounded-xl font-black uppercase text-xs transition-transform active:scale-95">Atrás</button>
                                        <button onClick={joinClan} className="flex-1 bg-purple-600 text-white py-3 rounded-xl font-black uppercase text-xs transition-transform active:scale-95 shadow-lg flex items-center justify-center gap-2"><Key size={14}/> Unirse</button>
                                    </div>
                                </div>
                            )}
                            ...
                            {clanSetupMode === 'create' && (
                                <div className="space-y-3 animate-in fade-in">
                                    ...
                                    <div className="flex gap-2 mt-2">
                                        <button onClick={() => setClanSetupMode(null)} className="flex-1 bg-zinc-800 text-zinc-400 py-3 rounded-xl font-black uppercase text-xs transition-transform active:scale-95">Atrás</button>
                                        <button onClick={createClan} className="flex-1 border border-purple-500 text-purple-400 py-3 rounded-xl font-black uppercase text-xs transition-transform active:scale-95 shadow-lg flex items-center justify-center gap-2"><Crown size={14}/> Fundar</button>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {userRole === 'leader' && (
                                <div className="bg-zinc-950 border border-zinc-800 p-3 rounded-2xl">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <button onClick={generateInviteCode} className="text-[9px] font-black uppercase bg-purple-600/20 text-purple-400 px-2 py-1 rounded border border-purple-500/30 transition-transform active:scale-95">+ Generar Código</button>
                                    </div>
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        {clanData.codes && clanData.codes.filter(c => !c.used).map(c => (
                                            <div key={c.code} className="bg-black border border-zinc-700 pl-3 pr-1 py-1 rounded-lg text-xs font-mono font-bold text-white flex items-center gap-2">
                                                <span onClick={() => copyToClipboard(c.code)} className="cursor-pointer flex items-center gap-1 transition-transform active:scale-95">{c.code} <Copy size={12} className="text-zinc-500"/></span>
                                                <div className="w-px h-4 bg-zinc-800 mx-1"></div>
                                                <button onClick={() => deleteInviteCode(c.code)} className="text-red-500 p-1.5 transition-transform active:scale-95 rounded"><X size={12}/></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                        ...
                                        <div key={m.uid} className="flex justify-between items-center p-2.5 border-b border-zinc-900/50">
                                            ...
                                            {userRole === 'leader' && m.uid !== user.uid && (
                                                <div className="flex gap-1.5">
                                                    <button onClick={() => toggleDelegate(m.uid)} className={`p-1.5 rounded-md border transition-transform active:scale-95 ${m.role === 'delegate' ? 'bg-blue-900/30 border-blue-500 text-blue-400' : 'bg-zinc-900 border-zinc-700 text-zinc-500'}`} title="Delegado"><UserCheck size={14}/></button>
                                                    <button onClick={() => kickMember(m.uid)} className="p-1.5 bg-red-900/20 border border-red-900/50 text-red-500 rounded-md transition-transform active:scale-95" title="Expulsar"><UserX size={14}/></button>
                                                </div>
                                            )}
                                        </div>
                                    ))}
                            ...
                            {(userRole === 'leader' || userRole === 'delegate') && (
                                <div className="bg-zinc-950 border border-purple-500/30 p-3 rounded-2xl space-y-2">
                                    ...
                                    <button onClick={sendPushToClan} className={`w-full py-2.5 rounded-lg font-black uppercase text-[10px] flex items-center justify-center gap-2 transition-transform active:scale-95 ${pushStatus === 'success' ? 'bg-emerald-600 text-white' : 'bg-red-600 text-white shadow-[0_0_10px_rgba(220,38,38,0.3)]'}`}>
                                        {pushStatus === 'success' ? <><Check size={14}/> Transmitido</> : <><Send size={14}/> Transmitir a todos</>}
                                    </button>
                                </div>
                            )}

                            <button onClick={leaveClan} className="w-full bg-zinc-900 text-zinc-400 border border-zinc-800 py-3 rounded-xl font-black uppercase text-[10px] transition-transform active:scale-95 flex items-center justify-center gap-2 mt-4">
      ...
      {showLogs && (
        <div className="fixed bottom-0 left-0 w-full h-64 bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800 z-[900] shadow-[0_-20px_50px_rgba(0,0,0,0.5)] flex flex-col animate-in slide-in-from-bottom-full duration-300">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-black/50">
                ...
                <button onClick={() => setShowLogs(false)} className="text-zinc-500 p-1 transition-transform active:scale-95 rounded-lg"><X size={16}/></button>
            </div>
      ...
      {activeHelp && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setActiveHelp(null)}>
          <div className="bg-zinc-900 border border-blue-500 p-6 rounded-3xl max-w-sm w-full relative shadow-[0_0_40px_rgba(59,130,246,0.3)] animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
             <button onClick={() => setActiveHelp(null)} className="absolute top-4 right-4 text-zinc-500 p-2 transition-transform active:scale-95 rounded-lg"><X size={20}/></button>
             ...
             <button onClick={() => setActiveHelp(null)} className="w-full mt-6 bg-blue-900/30 text-blue-400 py-3 rounded-xl font-black uppercase text-xs transition-transform active:scale-95">{t('understood')}</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md pointer-events-auto">
            <div className="bg-zinc-900 border border-amber-500 w-full max-w-sm rounded-[32px] shadow-[0_0_50px_rgba(245,158,11,0.15)] relative animate-in zoom-in duration-300 flex flex-col max-h-[85vh]">
               <div className="p-5 pb-3 border-b border-zinc-800 flex justify-between items-center shrink-0">
                   ...
                   <button onClick={() => { setShowSoundMenu(false); stopInfiniteAlarm(); }} className="text-zinc-500 p-1 transition-transform active:scale-95 rounded-lg"><X size={24}/></button>
               </div>
               <div className="overflow-y-auto p-4 space-y-6 custom-scrollbar">
                   ...
                   <div>
                       ...
                       <div className="space-y-1.5">
                           {SOUND_PROFILES.map(prof => (
                               <div key={`task-${prof.id}`} className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer transition-transform active:scale-95 ${taskSound === prof.id ? 'bg-amber-600/20 border-amber-500' : 'bg-zinc-800/50 border-zinc-800'}`} onClick={() => { setTaskSound(prof.id); playPreview(prof.id); if(syncRef.current) syncRef.current({ taskSound: prof.id }); }}>
      ...
      {confirmBoxReset && (
          <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md text-center">
            <div className="bg-zinc-900 w-full max-w-xs p-6 rounded-[32px] shadow-2xl animate-in zoom-in duration-300 border border-amber-500">
              ...
              <div className="flex gap-3"><button onClick={() => setConfirmBoxReset(null)} className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-black text-xs uppercase shadow-lg transition-transform active:scale-95">{t('cancel')}</button><button onClick={executeBoxReset} className="flex-1 bg-zinc-800 text-red-400 py-3 rounded-xl font-black text-xs uppercase transition-transform active:scale-95">{t('resetAll')}</button></div>
            </div>
          </div>
      )}
      ...
      <div className="max-w-md mx-auto space-y-4 pt-2">
        <div className="flex items-center justify-between bg-zinc-900/40 p-3 rounded-2xl border border-zinc-800 shadow-lg relative z-[50]">
          <div className="flex items-center gap-1.5">
            ...
            <button onClick={() => setActiveHelp('header')} className="text-zinc-600 p-1 transition-transform active:scale-95 rounded"><Info size={14}/></button>

            <button onClick={toggleLanguage} className="flex items-center gap-1 bg-zinc-800 border border-zinc-700 rounded-md px-1.5 py-1 xs:ml-2 transition-transform active:scale-95">
                <Globe size={12} className="text-zinc-400"/>
                <span className="text-[10px] font-black text-white uppercase">{lang}</span>
            </button>
            <button onClick={() => setShowLogs(!showLogs)} className={`ml-1 flex items-center p-1.5 rounded-md border transition-transform active:scale-95 ${showLogs ? 'bg-zinc-700 border-zinc-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                <TerminalSquare size={14}/>
            </button>
          </div>
          
          <div className="flex gap-1">
            <button onClick={() => setShowClanModal(true)} className={`p-1.5 rounded-lg border transition-transform active:scale-95 ${userClanId ? 'bg-purple-600 text-white border-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}><Crown size={18}/></button>
            <button onClick={toggleWakeLock} className={`p-1.5 rounded-lg border transition-transform active:scale-95 ${wakeLockActive ? 'bg-blue-600 text-white border-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}><Eye size={18} /></button>
            <button onClick={toggleSoundProfile} className={`p-1.5 rounded-lg border transition-transform active:scale-95 ${soundProfile !== 'muted' ? 'bg-amber-600 text-white border-amber-600 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>{soundProfile === 'siren' ? <Volume2 size={18}/> : soundProfile === 'radar' ? <Volume1 size={18}/> : <VolumeX size={18}/>}</button>
            <button onClick={() => setShowSoundMenu(true)} className="p-1.5 bg-zinc-800 text-amber-500 border border-zinc-700 rounded-lg shadow-lg transition-transform active:scale-95"><Music size={18}/></button>
            <button onClick={() => { const next = !vibrateOn; setVibrateOn(next); if(!next) stopInfiniteAlarm(); if(syncRef.current) syncRef.current({ vibrateOn: next }); addLog(`Vibración: ${next ? 'ON' : 'OFF'}`, 'info'); }} className={`p-1.5 rounded-lg border transition-transform active:scale-95 ${vibrateOn ? 'bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}><VibrateIcon size={18}/></button>
            <button onClick={toggleSystemNotifications} className={`p-1.5 rounded-lg border transition-transform active:scale-95 ${sysNotifOn ? 'bg-emerald-600 text-white border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>
                <Smartphone size={18}/>
            </button>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[32px] p-6 relative overflow-hidden shadow-2xl">
          ...
          <div className="pt-3 border-t border-zinc-800/50 flex justify-between items-end">
            <div className="flex items-center gap-1.5">
                ...
                <button onClick={() => setActiveHelp('time')} className="text-zinc-600 mb-1 transition-transform active:scale-95"><Info size={12}/></button>
            </div>
          ...
          <div className="mt-5 border-t border-zinc-800/50 pt-4">
              <div className="flex justify-between items-center mb-3">
                  ...
                  <button onClick={syncWar} className="bg-zinc-800 text-zinc-400 border border-zinc-700 text-[9px] font-black py-1 px-3 rounded-full flex items-center gap-1 transition-transform active:scale-95"><RotateCcw size={10} /> {t('sync')}</button>
              </div>
                      ...
                      if (a.custom) {
                          return (
                              <div key={a.id} className={`flex-[2] flex items-center justify-between rounded-xl border ${a.on ? 'bg-red-600 border-red-500' : 'bg-zinc-950 border-zinc-800'}`}>
                                  ...
                                  <button onClick={() => toggleAlarm(a.id)} className={`h-full px-2.5 rounded-r-xl border-l flex items-center justify-center transition-transform active:scale-95 ${a.on ? 'border-red-700 bg-red-700' : 'border-zinc-800'}`}><div className={`w-2 h-2 rounded-full ${a.on ? 'bg-white shadow-[0_0_5px_white]' : 'bg-zinc-700'}`} /></button>
                              </div>
                          );
                      }
                      return ( <button key={a.id} onClick={() => toggleAlarm(a.id)} className={`flex-1 rounded-xl text-xs font-black border transition-transform active:scale-95 ${a.on ? 'bg-red-600 text-white border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.3)]' : 'bg-zinc-950 text-zinc-500 border-zinc-800'}`}>{a.mins}M</button> );
                  })}
              </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 relative">
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                 <button onClick={() => setActiveHelp('creation')} className="bg-zinc-900 border border-zinc-700 text-zinc-500 p-1.5 rounded-full shadow-lg transition-transform active:scale-95"><Info size={14}/></button>
            </div>
            
            <button onClick={() => { setShowCronoForm(!showCronoForm); setShowBoxForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-transform active:scale-95 ${showCronoForm ? 'bg-amber-500 text-black border-amber-500' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
                {t('addCrono')}
            </button>
            <button onClick={() => { setShowBoxForm(!showBoxForm); setShowCronoForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 transition-transform active:scale-95 ${showBoxForm ? 'bg-blue-600 text-white border-blue-600' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
                {t('addSection')}
            </button>
        </div>
        ...
        {showCronoForm && (
            <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-3 space-y-3 animate-in slide-in-from-top-2">
                ...
                <button onClick={addCrono} className="w-full bg-amber-600 text-white rounded-xl py-3 font-black text-[10px] uppercase shadow-lg transition-transform active:scale-95">{t('createCronoBtn')}</button>
            </div>
        )}
        ...
        {showBoxForm && (
            <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-3 space-y-3 animate-in slide-in-from-top-2">
                ...
                <div className="flex items-center justify-between px-1">
                    ...
                    {includeInitialCrono && ( <div className="flex gap-2"><button onClick={() => changeDraftCount(true, false)} className="p-1 bg-zinc-800 rounded text-zinc-400 transition-transform active:scale-95"><Minus size={12}/></button><button onClick={() => changeDraftCount(true, true)} className="p-1 bg-zinc-800 rounded text-zinc-400 transition-transform active:scale-95"><Plus size={12}/></button></div> )}
                </div>
                ...
                <button onClick={addBox} className="w-full bg-blue-600 text-white rounded-xl py-3 font-black text-[10px] uppercase shadow-lg transition-transform active:scale-95">{t('createSectionBtn')}</button>
            </div>
        )}
                            ...
                            >
                                ...
                                {editingBoxId === box.id ? (
                                    <div className="flex-1 flex flex-col p-3 bg-zinc-900/90 w-full animate-in fade-in">
                                        <div className="flex gap-2 items-center w-full mb-3"><input className="min-w-0 w-full bg-zinc-800 text-xs font-black p-2 rounded outline-none border border-blue-500 uppercase text-white shadow-inner" value={editBuf.label} onChange={e => setEditBuf({...editBuf, label: e.target.value})} autoFocus placeholder={t('sectionNamePlaceholder')}/><div className="flex gap-1 shrink-0">{COLORS.map(c => <button key={c.hex} onClick={()=>setNewBoxColor(c.hex)} className={`w-5 h-5 rounded-full ${newBoxColor === c.hex ? 'border-2 border-white scale-110 shadow-lg' : 'opacity-40'}`} style={{backgroundColor:c.hex}}/>)}</div><div className="flex gap-1 shrink-0 border-l border-zinc-700 pl-2"><button onClick={() => { setEditingBoxId(null); setEditBoxDrafts([]); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm transition-transform active:scale-95"><X size={14}/></button><button onClick={() => saveBoxEdit(box.id)} className="p-2 bg-blue-600 rounded-lg text-white shadow-md transition-transform active:scale-95"><Check size={14}/></button></div></div>
                                        <div className="flex items-center justify-between px-1 py-2 border-t border-zinc-800/80"><span className="text-[10px] text-zinc-400 font-black uppercase tracking-widest flex items-center gap-1.5"><Plus size={10}/> {t('addCrono')}</span><div className="flex gap-1.5"><button onClick={() => handleEditBoxDraftCount(false)} className="p-1.5 bg-zinc-800 rounded-lg text-zinc-400 shadow-sm transition-transform active:scale-95"><Minus size={12}/></button><button onClick={() => handleEditBoxDraftCount(true)} className="p-1.5 bg-blue-600/20 text-blue-400 rounded-lg shadow-sm transition-transform active:scale-95"><Plus size={12}/></button></div></div>
                                        ...
                                        <div className="absolute right-3 flex items-center pointer-events-auto h-full">
                                            {isMenuOpen ? (
                                                <div className="flex items-center gap-1.5 animate-in slide-in-from-right-8 fade-in duration-200">
                                                    <button onClick={() => setConfirmBoxReset({id: box.id, name: box.name})} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm transition-transform active:scale-95" title="Reiniciar todos"><RotateCcw size={16}/></button><button onClick={() => handleBoxPlayPause(box.id, !allPausedOrFinished)} className={`p-2 bg-zinc-800 border border-zinc-700 rounded-lg shadow-sm transition-transform active:scale-95 ${!allPausedOrFinished ? 'text-zinc-400' : 'text-zinc-400'}`} title={!allPausedOrFinished ? 'Pausar todos' : 'Reanudar todos'}>{!allPausedOrFinished ? <Pause size={16}/> : <Play size={16}/>}</button><div className="w-px h-6 bg-zinc-700 mx-1"></div><button onClick={() => { setEditingBoxId(box.id); setEditBuf({label: box.name}); setNewBoxColor(box.color); setEditBoxDrafts([]); setOpenBoxMenuId(null); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm transition-transform active:scale-95"><Edit2 size={16}/></button><button onClick={async () => { const nb = boxes.filter(b => b.id !== box.id); let nr = rootOrder.filter(i => i.id !== box.id); const nt = tasks.map(t => { if (t.boxId === box.id) { nr.push({ id: t.id, type: 'task' }); return { ...t, boxId: null }; } return t; }); setBoxes(nb); setRootOrder(nr); setTasks(nt); if(syncRef.current) syncRef.current({boxes: nb, rootOrder: nr, tasks: nt}); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm transition-transform active:scale-95"><Trash2 size={16}/></button><button onClick={() => setOpenBoxMenuId(null)} className="p-2 ml-1 text-zinc-500 rounded-full transition-transform active:scale-95"><X size={18}/></button>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-1.5 animate-in fade-in"><button onClick={() => setOpenBoxMenuId(box.id)} className="p-2 text-zinc-500 rounded-lg cursor-pointer transition-transform active:scale-95"><Settings size={20} /></button><div className="w-px h-6 bg-zinc-800"></div><button onClick={() => toggleBoxCollapse(box.id)} className="p-2 text-zinc-400 rounded-lg cursor-pointer transition-transform active:scale-95">{box.isCollapsed ? <ChevronDown size={22}/> : <ChevronUp size={22}/>}</button></div>
                                            )}
                                        </div>
                                    </div>
                                )}
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md text-center">
            <div className={`w-full max-w-xs p-8 rounded-[40px] shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in zoom-in duration-300 border-4 flex flex-col relative ${currentAlertConf.bg} ${currentAlertConf.border}`}>
              ...
              <button 
                ...
                className={`w-full py-4 rounded-2xl font-black text-lg uppercase shadow-xl tracking-widest border border-black/30 transition-transform active:scale-95 ${currentAlertConf.btnBg} ${currentAlertConf.btnText}`}
              >
