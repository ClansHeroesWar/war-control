...
const App = () => {
  ...
  const SOUND_PROFILES = [
    ...
    { id: 'sonar', name: t('soundSonar'), type: t('typeCalm') }, { id: 'pulse', name: t('soundPulse'), type: t('typeCalm') },
    { id: 'chime', name: t('soundChime'), type: t('typeRelaxing') }, { id: 'crystal', name: t('soundCrystal'), type: t('typeRelaxing') },
    { id: 'zen', name: t('soundZen'), type: t('typeRelaxing') }, { id: 'muted', name: t('soundMuted'), type: t('typeSilent') }
  ];
      ...
      {confirmBoxReset && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md text-center">
            <div className="bg-zinc-900 w-full max-w-xs p-6 rounded-[32px] shadow-2xl animate-in zoom-in duration-300 border border-amber-500">
              ...
              <div className="flex gap-3"><button onClick={() => setConfirmBoxReset(null)} className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-black text-xs uppercase shadow-lg active:scale-95 transition-transform">{t('cancel')}</button><button onClick={executeBoxReset} className="flex-1 bg-zinc-800 text-zinc-400 py-3 rounded-xl font-black text-xs uppercase transition-colors active:bg-zinc-700 active:scale-95 active:text-red-500">{t('resetAll')}</button></div>
            </div>
          </div>
      )}
