...
// ==========================================
// SOLUCIÓN TÁCTIL NATIVA (Anti Ghost-Clicks)
// ==========================================
const BotonTactil = ({ children, onAccion, className = "", style = {} }) => {
  ...
  return (
    <button
      ...
      onPointerLeave={finalizarToque}
      onPointerCancel={finalizarToque}
      className={`transition-all duration-75 select-none outline-none ${presionado ? 'scale-95 brightness-75 bg-opacity-80' : 'scale-100'} ${className}`}
...
const dict = {
  en: { appTitle: "War Control", realTime: "Real Operational Time", endTime: "End Time", earlyWarnings: "Early Warnings", sync: "SYNC", addCrono: "Add Crono", addSection: "Create Section", createCronoTitle: "Create Timers", planNamePlaceholder: "PLAN NAME", activatePlan: "Activate Plan", createCronoBtn: "CREATE CRONO", createSectionBtn: "CREATE SECTION", sectionNamePlaceholder: "SECTION NAME", banner: "Banner:", includeCronos: "Include timers", emptySection: "Empty - Drop inside", exitZone: "Exit Zone", dropToExtract: "Drop to Extract", moving: "Moving", soundConfig: "Acoustic Config", taskFinished: "Plan Completed", warEndedTitle: "War Status", clanMessages: "Clan Messages", confirmResetTitle: "Confirm Reset", confirmResetDesc: "Reset all timers in section", cancel: "Cancel", resetAll: "Reset All", understood: "UNDERSTOOD", next: "NEXT", soundSiren: "Nuclear Siren", soundRadar: "Tactical Radar", soundAlert: "Standard Alert", soundDigital: "Digital Beep", soundSonar: "Submarine Sonar", soundChime: "Zen Chime", soundCrystal: "Crystal Echo", soundZen: "Harmonic Wave", soundMuted: "Muted", typeUrgent: "Urgent", typeCalm: "Calm", typeRelaxing: "Relaxing", typeSilent: "Silent", help_header_title: "Global Control Panel", help_header_desc: "• Globe: Changes system language.\n• Terminal: Opens Debug Logs.\n• Eye: Keeps the app running in the background.\n• Volume: System audio toggle.\n• Music Note: Acoustic settings.\n• Smartphone: Toggles system push notifications.", help_time_title: "Strategic Time Module", help_time_desc: "• End Time: Displays global target time.\n• Early Warnings: Select preset alerts.\n• H/M Inputs: Manually set target time.", help_creation_title: "Deployment Module", help_creation_desc: "• Add Crono: Injects independent timers to the field.\n• Create Section: Builds a visual folder." },
  es: { appTitle: "War Control", realTime: "Tiempo Real Operativo", endTime: "Hora Fin", earlyWarnings: "Avisos Tempranos de Guerra", sync: "SINCRONIZAR", addCrono: "Añadir Crono", addSection: "Crear Sección", createCronoTitle: "Crear Cronómetros", planNamePlaceholder: "NOMBRE DEL PLAN", activatePlan: "Activar Plan", createCronoBtn: "CREAR CRONO", createSectionBtn: "CREAR CAJA", sectionNamePlaceholder: "NOMBRE DE SECCIÓN", banner: "Banner:", includeCronos: "Incluir cronos", emptySection: "Vacío - Suelta adentro", exitZone: "Zona de Salida", dropToExtract: "Soltar para Extraer", moving: "Moviendo", soundConfig: "Config. Acústica", taskFinished: "Planes Finalizados", warEndedTitle: "Estado de la Guerra", clanMessages: "Mensajes de Clan", confirmResetTitle: "Confirmar Reinicio", confirmResetDesc: "Reinciar todos los cronos de la sección", cancel: "Cancelar", resetAll: "Reiniciar Todo", understood: "ENTENDIDO", next: "SIGUIENTE", soundSiren: "Sirena Nuclear", soundRadar: "Radar Táctico", soundAlert: "Alerta Estándar", soundDigital: "Bip Digital", soundSonar: "Sonar Submarino", soundChime: "Campana Zen", soundCrystal: "Cristal Eco", soundZen: "Onda Armónica", soundMuted: "Silenciado", typeUrgent: "Urgente", typeCalm: "Calma", typeRelaxing: "Relajante", typeSilent: "Silencio", help_header_title: "Panel de Control Global", help_header_desc: "• Globo: Cambia el idioma.\n• Terminal: Abre el registro de eventos.\n• Ojo: App en segundo plano.\n• Volumen: Silencia el audio general.\n• Nota Musical: Configuración de sonidos.\n• Teléfono: Activa notificaciones push.", help_time_title: "Módulo de Tiempo Estratégico", help_time_desc: "• Hora Fin: Límite global.\n• Avisos Tempranos: Calcula alertas.", help_creation_title: "Módulo de Despliegue", help_creation_desc: "• Añadir Crono: Inyecta cronómetros.\n• Crear Sección: Contenedores visuales." }
};
...
try { app = initializeApp(firebaseConfig); authInstance = getAuth(app); dbInstance = getFirestore(app); if (typeof window !== 'undefined') { messagingInstance = getMessaging(app); } isOfflineMode = false; } catch (error) { isOfflineMode = true; }
...
const App = () => {
  const appId = typeof __app_id !== 'undefined' ? __app_id : 'war-control-pro';
  const [lang, setLang] = useState('es');
  const t = useCallback((key) => dict[lang][key] || key, [lang]);
  const [activeHelp, setActiveHelp] = useState(null);
  
  // ==========================================
  // TERMINAL HIPER-SENSIBLE
  // ==========================================
  const [actionLog, setActionLog] = useState([]);
  const [showLogs, setShowLogs] = useState(false);

  const addLog = useCallback((msg, type = 'info') => {
    const timestamp = new Date().toISOString().slice(11, 23); // Incluye milisegundos
    setActionLog(prev => [{ time: timestamp, msg: String(msg), type }, ...prev].slice(0, 100)); // Capacidad aumentada a 100
  }, []);

  // Interceptor global de interacciones táctiles (Ignora movimientos continuos para no destruir la RAM)
  useEffect(() => {
    const handleGlobalTouch = (e) => {
        // Filtro para ignorar scroll o arrastre
        if (e.type === 'pointermove') return; 
        
        let targetStr = e.target.tagName.toLowerCase();
        if (e.target.textContent && e.target.textContent.length < 20) {
            targetStr += ` ("${e.target.textContent}")`;
        } else if (e.target.className && typeof e.target.className === 'string') {
            targetStr += ` (.${e.target.className.split(' ')[0]})`;
        }
        addLog(`Interacción física [${e.type}]: ${targetStr}`, 'interaccion');
    };

    window.addEventListener('pointerdown', handleGlobalTouch, { passive: true });
    return () => window.removeEventListener('pointerdown', handleGlobalTouch);
  }, [addLog]);

  const SOUND_PROFILES = [ { id: 'siren', name: t('soundSiren'), type: t('typeUrgent') }, { id: 'radar', name: t('soundRadar'), type: t('typeUrgent') }, { id: 'alert', name: t('soundAlert'), type: t('typeUrgent') }, { id: 'digital', name: t('soundDigital'), type: t('typeUrgent') }, { id: 'sonar', name: t('soundSonar'), type: t('typeCalm') }, { id: 'chime', name: t('soundChime'), type: t('typeRelaxing') }, { id: 'crystal', name: t('soundCrystal'), type: t('typeRelaxing') }, { id: 'zen', name: t('soundZen'), type: t('typeRelaxing') }, { id: 'muted', name: t('soundMuted'), type: t('typeSilent') } ];
  const ANTI_SLEEP_PROFILES = [ { id: 'engine', name: "Motor Submarino", type: "Zumbido Profundo (40Hz)" }, { id: 'heartbeat', name: "Latido Subliminal", type: "Pulso rítmico bajo" }, { id: 'static', name: "Estática de Radio", type: "Ruido blanco suave" }, { id: 'ultrasonic', name: "Ultrasonido (19kHz)", type: "Inaudible al humano" }, { id: 'null', name: "Onda Nula", type: "Silencio matemático" } ];
  ...
  const toggleLanguage = () => { const nextLang = lang === 'en' ? 'es' : 'en'; setLang(nextLang); if (syncRef.current) syncRef.current({ language: nextLang }); addLog(`Idioma del sistema alterado a ${nextLang.toUpperCase()}`, 'info'); };

  const startAntiSleepAudio = useCallback((type) => {
      initGlobalAudio(); if (!globalAudioCtx) return null; const ctx = globalAudioCtx; if (ctx.state === 'suspended') ctx.resume();
      const masterGain = ctx.createGain(); masterGain.connect(ctx.destination);
      let nodesToStop = []; let intervalId = null;

      addLog(`[AUDIO ENGINE] Arrancando espectro anti-suspensión: ${type}`, 'sistema');

      if (type === 'engine') {
          const osc = ctx.createOscillator(); osc.type = 'sine'; osc.frequency.value = 40; masterGain.gain.value = 0.1; osc.connect(masterGain); osc.start(); nodesToStop.push(osc);
      } else if (type === 'static') {
          ...
          const noise = ctx.createBufferSource(); noise.buffer = buffer; noise.loop = true; masterGain.gain.value = 0.005; noise.connect(masterGain); noise.start(); nodesToStop.push(noise);
      } else if (type === 'heartbeat') {
          masterGain.gain.value = 0.4;
          const playBeat = () => { if (ctx.state === 'closed') return; const osc = ctx.createOscillator(); const gain = ctx.createGain(); osc.connect(gain); gain.connect(masterGain); osc.type = 'sine'; osc.frequency.value = 40; gain.gain.setValueAtTime(0, ctx.currentTime); gain.gain.linearRampToValueAtTime(0.3, ctx.currentTime + 0.1); gain.gain.exponentialRampToValueAtTime(0.001, ctx.currentTime + 0.4); osc.start(ctx.currentTime); osc.stop(ctx.currentTime + 0.5); nodesToStop.push(osc); };
          playBeat(); intervalId = setInterval(() => { playBeat(); setTimeout(playBeat, 400); }, 1500);
      } else if (type === 'ultrasonic') {
          const osc = ctx.createOscillator(); osc.type = 'sine'; osc.frequency.value = 19000; masterGain.gain.value = 0.05; osc.connect(masterGain); osc.start(); nodesToStop.push(osc);
      } else if (type === 'null') {
          const osc = ctx.createOscillator(); osc.type = 'sine'; osc.frequency.value = 100; masterGain.gain.value = 0.0001; osc.connect(masterGain); osc.start(); nodesToStop.push(osc);
      }

      return { stop: () => { if (intervalId) clearInterval(intervalId); nodesToStop.forEach(n => { try { n.stop(); n.disconnect(); } catch(e){} }); try { masterGain.disconnect(); } catch(e){} } };
  }, [addLog]);

  const toggleWakeLock = async () => {
    addLog(`[HARDWARE] Solicitud de cambio WakeLock actual: ${wakeLockActive}`, 'sistema');
    if (wakeLockActive) {
      if (wakeLockRef.current) { try { await wakeLockRef.current.release(); } catch(e){} wakeLockRef.current = null; }
      if (antiSleepEngineRef.current) { antiSleepEngineRef.current.stop(); antiSleepEngineRef.current = null; }
      setWakeLockActive(false); addLog("Modo Vigía y Motor Anti-Suspensión DESACTIVADOS.", "warning"); return;
    }
    antiSleepEngineRef.current = startAntiSleepAudio(antiSleepSound);
    if (!('wakeLock' in navigator)) { setWakeLockActive(true); addLog("Modo Vigía: API WakeLock no soportada, forzando audio silencioso continuo.", "warning"); return; }
    try {
      wakeLockRef.current = await navigator.wakeLock.request('screen'); setWakeLockActive(true); addLog("Modo Vigía Completo ACTIVADO (Audio + Pantalla).", "success");
      wakeLockRef.current.addEventListener('release', () => { addLog("ADVERTENCIA OS: Pantalla sin bloqueo, confiando en Motor de Audio.", "warning"); });
    } catch (err) { setWakeLockActive(true); addLog(`WakeLock falló por permisos OS. Forzando Audio Continuo.`, "error"); }
  };

  useEffect(() => { if (wakeLockActive) { if (antiSleepEngineRef.current) antiSleepEngineRef.current.stop(); antiSleepEngineRef.current = startAntiSleepAudio(antiSleepSound); } }, [antiSleepSound, wakeLockActive, startAntiSleepAudio]);

  useEffect(() => {
    const handleVisibilityChange = () => {
      addLog(`[SISTEMA] Cambio estado visual: ${document.visibilityState}`, 'sistema');
      if (document.visibilityState === 'visible') { setTasks(prev => [...prev]); if (wakeLockActive && wakeLockRef.current === null && 'wakeLock' in navigator) { navigator.wakeLock.request('screen').then(lock => { wakeLockRef.current = lock; }).catch(()=>{}); } }
    };
    document.addEventListener('visibilitychange', handleVisibilityChange);
    return () => document.removeEventListener('visibilitychange', handleVisibilityChange);
  }, [wakeLockActive, addLog]);

  const triggerHaptic = useCallback((pattern) => { if (!navigator.vibrate) return; try { navigator.vibrate(pattern); addLog(`[HARDWARE] Motor vibratorio disparado`, 'sistema'); } catch (e) {} }, [addLog]);
  ...
  const stopInfiniteAlarm = useCallback(() => {
      ...
      if (previewEngineRef.current) { previewEngineRef.current.stop(); previewEngineRef.current = null; }
      addLog(`[AUDIO] Todo sonido externo/alarma detenido.`, 'info');
  }, [addLog]);

  const synthesizeAudio = useCallback((profile, isPreview = false) => {
      if (profile === 'muted') return null; initGlobalAudio(); if (!globalAudioCtx) return null;
      try {
          ...
          const scheduleNote = (type, freq, timeOffset, vol, attack=0.05, decay=0.1) => {
              if(ctx.state === 'closed') return; const osc = ctx.createOscillator(); const gain = ctx.createGain();
          ...
          const playSequence = () => {
              ...
              switch(profile) {
                  ...
                  case 'chime': scheduleNote('sine', 600, 0, 0.5, 0.05, 2.0); scheduleNote('sine', 900, 0, 0.2, 0.05, 2.0); break;
  ...
  const playPreview = useCallback((profile, isAntiSleepPreview = false) => {
      ...
      if (profile === 'muted') { addLog("Perfil de alerta configurado a: SILENCIADO", "info"); return; }
      addLog(`[AUDIO] Reproduciendo muestra temporal: ${profile}`, "info");
      if (isAntiSleepPreview) { previewEngineRef.current = startAntiSleepAudio(profile); setTimeout(() => { if (previewEngineRef.current) { previewEngineRef.current.stop(); previewEngineRef.current = null; } }, 3000); } else { previewEngineRef.current = synthesizeAudio(profile, true); }
  ...
  const triggerInfiniteAlarm = useCallback((type) => {
      ...
      addLog(`ALERTA TÁCTICA DISPARADA [Causa: ${type}]. Empleando acústica: ${profile}`, "warning");
  ...
  }, [stopInfiniteAlarm, triggerHaptic, synthesizeAudio, addLog]);
  ...
  const toggleSystemNotifications = async () => {
      if (sysNotifOn) { setSysNotifOn(false); if (syncRef.current) syncRef.current({ sysNotifOn: false }); addLog("Notificaciones locales API/Push DESACTIVADAS.", "warning"); return; }
      if (typeof Notification === 'undefined') { addLog("API Web Notification no disponible en el compilador Android actual.", "error"); return; }
      try {
          const status = await Notification.requestPermission();
          if (status === 'granted') {
              setSysNotifOn(true); if (syncRef.current) syncRef.current({ sysNotifOn: true }); addLog("Permisos concedidos. Alertas de sistema habilitadas.", "success");
              if (messagingInstance && user) { try { const currentToken = await getToken(messagingInstance, { vapidKey: VAPID_KEY }); if (currentToken) { setPushToken(currentToken); if (syncRef.current) syncRef.current({ fcmToken: currentToken }); addLog("Token FCM generado exitosamente.", "success"); } } catch (e) { addLog(`Fallo Cloud Messaging, pero alertas locales siguen activas.`, "warning"); } }
          } else { addLog("Permiso de notificaciones bloqueado por OS/Usuario.", "error"); }
      } catch (err) { addLog(`Error al conectar servicio Notificaciones: ${err.message}`, "error"); }
  };

  const triggerSystemNotification = useCallback(async (title, body) => {
      if (!sysNotifOn || typeof Notification === 'undefined' || Notification.permission !== 'granted') return;
      try {
          addLog(`[PUSH] Disparando notificación al SO: ${title}`, 'sistema');
          if ('serviceWorker' in navigator) {
              const regs = await navigator.serviceWorker.getRegistrations();
              if (regs.length > 0) { await regs[0].showNotification(String(title), { body: String(body), icon: 'https://cdn-icons-png.flaticon.com/512/1041/1041916.png', badge: 'https://cdn-icons-png.flaticon.com/512/1041/1041916.png', vibrate: [500, 200, 500, 200, 500], requireInteraction: true }); return; }
          }
          new Notification(String(title), { body: String(body), requireInteraction: true });
      } catch (e) { addLog(`Fallo al inyectar bandeja Android: ${e.message}`, "error"); }
  }, [sysNotifOn, addLog]);

  useEffect(() => {
    if (isOfflineMode) {
        addLog("[CONEXIÓN] Sin acceso Firebase. Fallback a Memoria Local.", "warning");
        ...
        if (rawData) {
            try {
                ...
                if (data.fcmToken) setPushToken(data.fcmToken);
                if (data.antiSleepSound) setAntiSleepSound(data.antiSleepSound);
                if (data.tasks) {
                  const now = Date.now();
            ...
            } catch(e) { addLog("Corrupción en memoria local", "error"); }
        }
        setIsLoaded(true); return;
    }

    const initAuth = async () => { if (authInstance) { try { await signInAnonymously(authInstance); onAuthStateChanged(authInstance, (currentUser) => { setUser(currentUser); addLog(`Usuario anónimo vinculado: ${currentUser.uid.substring(0,6)}...`, 'success'); }); } catch(e) { addLog(`Bloqueo de Autenticación de Red: ${e.message}`, "error"); } } };
    initAuth();
  }, [addLog]);
  ...
  useEffect(() => {
    ...
    addLog(`[RED] Suscribiendo a nodo remoto: ${user.uid.substring(0,6)}`, 'info');
    
    const unsub = onSnapshot(docRef, (snap) => {
      if (snap.exists() && !isDraggingRef.current) { 
        try {
            addLog("[RED] Paquete de sincronización recibido", "sistema");
        ...
        } catch (e) { addLog("Error de parsing durante sincronización de red", "error"); }
      }
      setIsLoaded(true);
    }, (error) => addLog(`Fallo de conexión Firestore: ${error.message}`, 'error'));
    return () => unsub();
  }, [user, addLog]);

  const sync = async (updates) => {
    if (isOfflineMode) {
        addLog("[LOCAL] Guardando mutaciones en disco", "sistema");
        const current = JSON.parse(localStorage.getItem('war_control_data_v45') || '{}');
        localStorage.setItem('war_control_data_v45', JSON.stringify({ ...current, ...updates })); return;
    }
    if (!user || !dbInstance) return;
    addLog("[RED] Despachando mutación a Firestore", "sistema");
    const docRef = doc(dbInstance, 'artifacts', appId, 'users', user.uid, 'settings', 'global_data_v45');
    await setDoc(docRef, updates, { merge: true }).catch(e => addLog(`Fallo subida a nube: ${e.message}`, "error"));
  };

  useEffect(() => { syncRef.current = sync; }, [user, isOfflineMode, addLog]);
  ...
  // ==========================================
  // BUCLE DE ESTADO TEMPORAL (Restringido para no abusar de React)
  // ==========================================
  useEffect(() => {
      const ticker = setInterval(() => {
          ...
          const nextTasks = tasksRef.current.map(t => {
            if (t.isRunning && t.serverEndTime) {
              ...
              if (exactRemaining === 0 && !t.alerted) {
                  ...
                  addLog(`¡CRONO DETONADO!: ${t.label}`, 'warning');
          ...
          if (targetEndTimeRef.current) {
            try {
              ...
              if (!isNaN(targetTimeMs)) {
                  ...
                  if (visualSecsRem === 0 && !globalWarAlertedRef.current) { globalWarAlertedRef.current = true; warEndedNow = true; addLog(`*** GUERRA GLOBAL FINALIZADA ***`, 'warning'); } 
                  ...
                  if (msRem < 86400000 && msRem > -86400000) { 
                      warAlarmsRef.current.forEach(a => {
                          if (a.on && !a.trig) {
                              ...
                              if (limitSecs > 0 && visualSecsRem <= limitSecs && visualSecsRem >= 0) { newlyTriggeredAlarms.push(a); addLog(`[ESTRATEGIA] Alerta temprana superada: límite de ${limitSecs}s`, "info"); }
                          }
                      });
                  }
              }
            } catch(e) { addLog("Fallo crítico en cálculo de guerra maestro", "error"); }
          }
          ...
          if (newlyTriggeredAlarms.length > 0 || warEndedNow) {
              let alertTitle = t('warEndedTitle'); let notifBody = "";
              if (warEndedNow) { notifBody += `Guerra finalizada.\n\n`; }

              if (newlyTriggeredAlarms.length > 0) {
                  const alarmLabels = newlyTriggeredAlarms.map(a => { let timeStr = a.custom ? `${a.h ? a.h+'h ' : ''}${a.m ? a.m+'m ' : ''}${a.s ? a.s+'s' : ''}`.trim() : `${a.mins} Minutos`; return `La guerra finaliza en ${timeStr}`; }).join('\n• ');
      ...
      }, 500); // 500ms es el límite saludable para React en dispositivos móviles para no asfixiar el UI thread
  ...
  const syncWar = async () => {
    addLog(`Ejecución sincronización de Guerra Maestro`, "interaccion");
    ...
    const rA = warAlarms.map(a => { let limitMs = a.custom ? (((parseInt(a.h)||0)*3600) + ((parseInt(a.m)||0)*60) + (parseInt(a.s)||0))*1000 : a.mins*60000; if (a.on && msRem <= limitMs) return { ...a, trig: true }; return { ...a, trig: false }; });
    setWarAlarms(rA); if(syncRef.current) syncRef.current({ targetEndTime: end.getTime(), warAlarms: rA });
  };

  const toggleAlarm = (id) => {
      addLog(`Alteración de estado en aviso de guerra: ID ${id}`, "interaccion");
      ...
      const next = warAlarms.map(a => {
          if (a.id === id) { const isTurningOn = !a.on; let limitMs = a.custom ? (((parseInt(a.h)||0)*3600) + ((parseInt(a.m)||0)*60) + (parseInt(a.s)||0))*1000 : a.mins*60000; let newTrig = a.trig; if (isTurningOn) { if (targetEndTimeRef.current && !isNaN(targetEndTimeRef.current.getTime()) && msRem <= limitMs) { newTrig = true; } else { newTrig = false; } } return { ...a, on: isTurningOn, trig: newTrig }; }
          return a;
      });
  ...
  const addCrono = async () => {
    addLog(`Añadiendo ${cronoDrafts.length} nuevos cronómetros al campo`, "interaccion");
    let nlTasks = [...tasks], nlRootOrder = [...rootOrder], baseTime = Date.now(), startingIndex = tasks.length + 1;
    cronoDrafts.forEach((draft, index) => { const h = parseInt(draft.h || 0), m = parseInt(draft.m || 0), s = parseInt(draft.s || 0); const total = (h * 3600) + (m * 60) + s; const label = (draft.label.trim() || `PLAN ${startingIndex + index}`).toUpperCase(); nlTasks.push({ id: baseTime + index, label, initialSeconds: total, remainingSeconds: total, serverEndTime: baseTime + (total * 1000), isRunning: true, boxId: null, isNewFinish: false, alerted: false }); nlRootOrder.push({ id: baseTime + index, type: 'task' }); });
  ...
  const addBox = async () => {
    addLog(`Creación estructural de nueva sección/caja`, "interaccion");
    const boxId = "box_" + Date.now(); const newBox = { id: boxId, name: (newBoxLabel.trim() || `SECTION ${boxes.length + 1}`).toUpperCase(), color: newBoxColor, isCollapsed: false }; let nlBoxes = [...boxes, newBox], nlTasks = [...tasks], nlRootOrder = [...rootOrder, {id: boxId, type: 'box'}]; const baseTime = Date.now(), startingIndex = tasks.length + 1;
    if (includeInitialCrono) { boxCronoDrafts.forEach((draft, index) => { const h = parseInt(draft.h || 0), m = parseInt(draft.m || 0), s = parseInt(draft.s || 0); const total = (h * 3600) + (m * 60) + s; const label = (draft.label.trim() || `PLAN ${startingIndex + index}`).toUpperCase(); nlTasks.push({ id: baseTime + 1000 + index, label, initialSeconds: total, remainingSeconds: total, serverEndTime: baseTime + (total * 1000), isRunning: true, boxId: boxId, isNewFinish: false, alerted: false }); }); }
    setBoxes(nlBoxes); setTasks(nlTasks); setRootOrder(nlRootOrder); if(syncRef.current) syncRef.current({ boxes: nlBoxes, tasks: nlTasks, rootOrder: nlRootOrder }); setNewBoxLabel(''); setBoxCronoDrafts([{ id: Date.now(), label: '', h: '', m: '', s: '' }]); setShowBoxForm(false); setIncludeInitialCrono(false);
  };

  const saveBoxEdit = async (boxId) => {
    addLog(`Guardado de reestructuración en sección ${boxId}`, "interaccion");
    const updatedName = editBuf.label.trim().toUpperCase() || "SECTION"; const nb = boxes.map(b => b.id === boxId ? { ...b, name: updatedName, color: newBoxColor } : b); let nlTasks = [...tasks], baseTime = Date.now(), existingCount = tasks.filter(t => t.boxId === boxId).length;
    editBoxDrafts.forEach((draft, index) => { const total = ((parseInt(draft.h || 0) * 3600) + (parseInt(draft.m || 0) * 60) + parseInt(draft.s || 0)); if (total > 0) { const label = (draft.label.trim() || `NEW ${existingCount + index + 1}`).toUpperCase(); nlTasks.push({ id: baseTime + index, label, initialSeconds: total, remainingSeconds: total, serverEndTime: baseTime + (total * 1000), isRunning: true, boxId: boxId, isNewFinish: false, alerted: false }); } });
  ...
  const saveEdit = async (id) => {
    addLog(`Aplicando parche temporal a crono ID: ${id}`, "interaccion");
    const ns = ((parseInt(editBuf.h) || 0) * 3600) + ((parseInt(editBuf.m) || 0) * 60) + ((parseInt(editBuf.s) || 0));
    const nl = tasks.map(t => { if (t.id === id) { const timeChanged = t.initialSeconds !== ns; return { ...t, label: editBuf.label.trim().toUpperCase() || t.label, initialSeconds: ns, remainingSeconds: timeChanged ? ns : t.remainingSeconds, serverEndTime: timeChanged ? Date.now()+(ns*1000) : t.serverEndTime, isRunning: timeChanged ? true : t.isRunning, isNewFinish: timeChanged ? false : t.isNewFinish, alerted: timeChanged ? false : t.alerted }; } return t; });
    setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); setEditingId(null);
  };

  const handleBoxPlayPause = (boxId, forcePause) => { addLog(`Mutación forzada de pausa/arranque en lote para sección ${boxId}`, "interaccion"); const nl = tasks.map(t => { if (t.boxId === boxId && t.remainingSeconds > 0) { const isRunning = forcePause ? false : true; return { ...t, isRunning, serverEndTime: isRunning ? Date.now() + (t.remainingSeconds * 1000) : null }; } return t; }); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); };
  const executeBoxReset = () => { if (!confirmBoxReset) return; addLog(`Purgando memoria de la sección completa: ${confirmBoxReset.id}`, "interaccion"); const boxId = confirmBoxReset.id; const nl = tasks.map(t => { if (t.boxId === boxId) { return { ...t, remainingSeconds: t.initialSeconds, serverEndTime: Date.now() + (t.initialSeconds * 1000), isRunning: true, isNewFinish: false, alerted: false }; } return t; }); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); setConfirmBoxReset(null); };
  const toggleBoxCollapse = (boxId) => { addLog(`Colapso visual de bloque ID: ${boxId}`, "interaccion"); const nb = boxes.map(b => b.id === boxId ? { ...b, isCollapsed: !b.isCollapsed } : b); setBoxes(nb); if(syncRef.current) syncRef.current({ boxes: nb }); };
  ...
  const handleItemPointerDown = (e, id, type) => {
    ...
    const timerId = setTimeout(() => {
        addLog(`[DRAG_START] Levantamiento de objeto anclado al motor DND. Tipo: ${type}`, "sistema");
  ...
  const executePointerDrop = async () => {
    ...
    addLog(`[DRAG_DROP] Objeto asentado. Objetivo: ${dropInd ? dropInd.id : 'Nulo'}`, 'sistema');
  ...
  const dismissNewFinish = (id) => { addLog(`Ocultando flasheo visual de finalización de crono ID: ${id}`, 'interaccion'); const nl = tasks.map(x => x.id === id ? { ...x, isNewFinish: false } : x); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); };

  const warRem = (() => { if (!targetEndTime || isNaN(targetEndTime.getTime())) return { h: 0, m: 0, s: 0 }; const d = targetEndTime.getTime() - currentTime.getTime(); if (d <= 0) return { h: 0, m: 0, s: 0 }; return { h: Math.floor(d/3600000), m: Math.floor((d%3600000)/60000), s: Math.floor((d%60000)/1000) }; })();
  let globalTimePart = "--:--", globalAmpmPart = "--"; if (targetEndTime && !isNaN(targetEndTime.getTime())) { try { const timeParts = targetEndTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', hour12: true }).split(' '); globalTimePart = timeParts[0] || "--:--"; globalAmpmPart = timeParts[1] || ''; } catch (e) {} }
      ...
      >
        ...
        <div className="flex items-center justify-between pointer-events-none mt-1">
          ...
          <div className="flex items-center gap-1 border-l border-zinc-800 pl-2 shrink-0 pointer-events-auto h-full">
            {editingId === t.id ? (
              <div className="flex gap-1"><BotonTactil onAccion={() => setEditingId(null)} className="p-2 bg-zinc-700/80 rounded-lg text-white"><X size={16}/></BotonTactil><BotonTactil onAccion={() => saveEdit(t.id)} className="p-2 bg-blue-600 rounded-lg text-white shadow-md shadow-blue-900/50"><Check size={16}/></BotonTactil></div>
            ) : (
              <>
                {t.isNewFinish && ( <BotonTactil onAccion={(e) => { e.stopPropagation(); dismissNewFinish(t.id); }} className="p-1 px-2.5 bg-amber-600 text-white rounded-lg font-black text-[9px] uppercase flex items-center gap-1 animate-pulse mr-1 shadow-[0_0_10px_rgba(245,158,11,0.5)]"><RolledScrollIcon size={12}/> Visto</BotonTactil> )}
                <BotonTactil onAccion={async () => { const nl = tasks.map(x => x.id === t.id ? { ...x, remainingSeconds: x.initialSeconds, serverEndTime: Date.now() + (x.initialSeconds * 1000), isRunning: true, isNewFinish: false, alerted: false } : x); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); }} className={`p-1.5 rounded-lg border border-transparent hover:bg-white/5 ${t.isNewFinish ? 'text-amber-300' : 'text-zinc-700'}`}><RotateCcw size={16} /></BotonTactil>
                <BotonTactil onAccion={() => { const hVal = Math.floor(t.initialSeconds / 3600); const mVal = Math.floor((t.initialSeconds % 3600) / 60); const sVal = t.initialSeconds % 60; setEditBuf({ label: t.label, h: hVal > 0 ? String(hVal) : '', m: mVal > 0 ? String(mVal) : '', s: sVal > 0 ? String(sVal) : '' }); setEditingId(t.id); }} className={`p-1.5 rounded-lg border border-transparent hover:bg-white/5 ${t.isNewFinish ? 'text-amber-400' : 'text-zinc-700'}`}><Edit2 size={16} /></BotonTactil>
                <BotonTactil onAccion={async () => { const nl = tasks.map(x => x.id === t.id ? { ...x, isRunning: !x.isRunning, serverEndTime: !x.isRunning ? Date.now() + (x.remainingSeconds * 1000) : null } : x); setTasks(nl); if(syncRef.current) syncRef.current({ tasks: nl }); }} className={`p-1.5 rounded-lg border border-transparent hover:bg-white/5 ${!t.isRunning && t.remainingSeconds > 0 ? 'text-yellow-400' : (t.isNewFinish ? 'text-amber-300' : 'text-zinc-600')}`}><Pause size={18} className={t.isRunning && t.remainingSeconds > 0 ? 'block' : 'hidden'}/><Play size={18} className={!t.isRunning && t.remainingSeconds > 0 ? 'block' : 'hidden'} /></BotonTactil>
                <BotonTactil onAccion={async () => { const nt = tasks.filter(x => x.id !== t.id); const nr = rootOrder.filter(item => item.id !== t.id); setTasks(nt); setRootOrder(nr); if(syncRef.current) syncRef.current({ tasks: nt, rootOrder: nr }); }} className={`p-1.5 rounded-lg border border-transparent hover:bg-white/5 ${t.isNewFinish ? 'text-amber-500' : 'text-zinc-800'}`}><Trash2 size={16} /></BotonTactil>
              </>
            )}
          </div>
        </div>
  ...
  const alertConfig = { war: { bg: 'bg-red-600', border: 'border-red-400', text: 'text-red-950', btnBg: 'bg-red-950', btnText: 'text-red-400', icon: <HornIcon size={64} className="mx-auto mb-4 animate-bounce text-red-950"/> }, task: { bg: 'bg-amber-500', border: 'border-amber-300', text: 'text-amber-950', btnBg: 'bg-amber-950', btnText: 'text-amber-400', icon: <RolledScrollIcon size={64} className="mx-auto mb-4 animate-bounce text-amber-950"/> }, clan: { bg: 'bg-purple-600', border: 'border-purple-400', text: 'text-purple-950', btnBg: 'bg-purple-950', btnText: 'text-purple-400', icon: <CrownScrollIcon size={64} className="mx-auto mb-4 animate-bounce text-purple-950"/> } };
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 p-4 pb-32 overflow-x-hidden font-sans">
      <style>{` * { user-select: none; -webkit-user-select: none; -webkit-touch-callout: none; -webkit-tap-highlight-color: transparent !important; outline: none !important; touch-action: manipulation; } *:focus, *:active { outline: none !important; -webkit-tap-highlight-color: transparent !important; } input { user-select: text; -webkit-user-select: text; -webkit-touch-callout: default; touch-action: auto; } @keyframes tremble { 0% { transform: rotate(1.5deg) scale(1.05); } 50% { transform: rotate(-1.5deg) scale(1.05); } 100% { transform: rotate(1.5deg) scale(1.05); } } .is-ghost { animation: tremble 0.12s infinite !important; } .drop-inside-target { background-color: rgba(245, 158, 11, 0.08) !important; border-color: #f59e0b !important; } .drop-extract-target { background-color: rgba(239, 68, 68, 0.15) !important; border-color: #ef4444 !important; } .custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: #3f3f46; border-radius: 4px; } `}</style>

      {showLogs && (
        <div className="fixed bottom-0 left-0 w-full h-80 bg-zinc-950/95 backdrop-blur-xl border-t border-zinc-800 z-[900] shadow-[0_-20px_50px_rgba(0,0,0,0.5)] flex flex-col animate-in slide-in-from-bottom-full duration-300">
            <div className="flex items-center justify-between px-4 py-3 border-b border-zinc-800 bg-black/50">
                <div className="flex items-center gap-2"><Activity size={16} className="text-zinc-500"/><span className="text-xs font-black uppercase text-zinc-400 tracking-widest">Terminal de Registro Hiper-Sensible</span></div>
                <BotonTactil onAccion={() => setShowLogs(false)} className="text-zinc-500 p-2"><X size={16}/></BotonTactil>
            </div>
            <div className="flex-1 overflow-y-auto p-4 space-y-2 font-mono text-[9px] sm:text-[10px] custom-scrollbar bg-[#050505]">
                {actionLog.map((log, i) => (
                    <div key={i} className="flex gap-3 leading-tight border-b border-zinc-900/50 pb-1.5 pt-1 hover:bg-zinc-900/30">
                        <span className="text-zinc-600 shrink-0 font-bold">[{log.time}]</span>
                        <span className={`${log.type === 'error' ? 'text-red-400 font-bold' : log.type === 'success' ? 'text-emerald-400' : log.type === 'warning' ? 'text-amber-400 font-bold' : log.type === 'interaccion' ? 'text-purple-400' : log.type === 'sistema' ? 'text-blue-500' : 'text-zinc-300'}`}>{log.msg}</span>
                    </div>
                ))}
                {actionLog.length === 0 && <p className="text-zinc-600 italic text-center mt-10">Escuchando mutaciones estructurales...</p>}
            </div>
        </div>
      )}
      ...
      {activeHelp && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200" onClick={() => setActiveHelp(null)}>
          <div className="bg-zinc-900 border border-blue-500 p-6 rounded-3xl max-w-sm w-full relative shadow-[0_0_40px_rgba(59,130,246,0.3)] animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
             <BotonTactil onAccion={() => setActiveHelp(null)} className="absolute top-4 right-4 text-zinc-500 p-2"><X size={20}/></BotonTactil>
             ...
             <BotonTactil onAccion={() => setActiveHelp(null)} className="w-full mt-6 bg-blue-900/30 text-blue-400 py-3 rounded-xl font-black uppercase text-xs text-center border border-blue-500/30 block">{t('understood')}</BotonTactil>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md pointer-events-auto">
            <div className="bg-zinc-900 border border-amber-500 w-full max-w-sm rounded-[32px] shadow-[0_0_50px_rgba(245,158,11,0.15)] relative animate-in zoom-in duration-300 flex flex-col max-h-[85vh]">
               <div className="p-5 pb-3 border-b border-zinc-800 flex justify-between items-center shrink-0">
                   ...
                   <BotonTactil onAccion={() => { setShowSoundMenu(false); stopInfiniteAlarm(); }} className="text-zinc-500 p-2 bg-zinc-800 rounded-lg"><X size={20}/></BotonTactil>
               </div>
                       ...
                       <div className="space-y-1.5">
                           {SOUND_PROFILES.map(prof => (
                               <div key={`war-${prof.id}`} className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer ${warSound === prof.id ? 'bg-amber-600/20 border-amber-500' : 'bg-zinc-800/50 border-zinc-800'}`} onClick={() => { setWarSound(prof.id); playPreview(prof.id); if(syncRef.current) syncRef.current({ warSound: prof.id }); }}>
                                   <div className="flex items-center gap-3"><BotonTactil onAccion={(e) => { e.stopPropagation(); playPreview(prof.id); }} className="p-1.5 bg-zinc-950 rounded-lg text-zinc-400"><Play size={12}/></BotonTactil><div><span className={`block text-xs font-black uppercase ${warSound === prof.id ? 'text-amber-500' : 'text-zinc-300'}`}>{prof.name}</span><span className="block text-[8px] text-zinc-500 font-bold uppercase">{prof.type}</span></div></div>{warSound === prof.id && <Check size={16} className="text-amber-500 mr-2"/>}
                               </div>
                           ))}
                       </div>
                   </div>
                       ...
                       <div className="space-y-1.5">
                           {ANTI_SLEEP_PROFILES.map(prof => (
                               <div key={`antisleep-${prof.id}`} className={`flex items-center justify-between p-2.5 rounded-xl border cursor-pointer ${antiSleepSound === prof.id ? 'bg-blue-600/20 border-blue-500' : 'bg-zinc-800/50 border-zinc-800'}`} onClick={() => { setAntiSleepSound(prof.id); playPreview(prof.id, true); if(syncRef.current) syncRef.current({ antiSleepSound: prof.id }); }}>
                                   <div className="flex items-center gap-3"><BotonTactil onAccion={(e) => { e.stopPropagation(); playPreview(prof.id, true); }} className="p-1.5 bg-zinc-950 rounded-lg text-zinc-400"><Play size={12}/></BotonTactil><div><span className={`block text-xs font-black uppercase ${antiSleepSound === prof.id ? 'text-blue-500' : 'text-zinc-300'}`}>{prof.name}</span><span className="block text-[8px] text-zinc-500 font-bold uppercase">{prof.type}</span></div></div>{antiSleepSound === prof.id && <Check size={16} className="text-blue-500 mr-2"/>}
                               </div>
                           ))}
                       </div>
                   </div>
      ...
      {confirmBoxReset && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md text-center">
            <div className="bg-zinc-900 w-full max-w-xs p-6 rounded-[32px] shadow-2xl animate-in zoom-in duration-300 border border-amber-500">
              ...
              <div className="flex gap-3"><BotonTactil onAccion={() => setConfirmBoxReset(null)} className="flex-1 bg-amber-600 text-white py-3 rounded-xl font-black text-xs uppercase shadow-lg text-center block">{t('cancel')}</BotonTactil><BotonTactil onAccion={executeBoxReset} className="flex-1 bg-zinc-800 text-red-400 py-3 rounded-xl font-black text-xs uppercase text-center block">{t('resetAll')}</BotonTactil></div>
            </div>
          </div>
      )}
      ...
      <div className="max-w-md mx-auto space-y-4 pt-2">
        <div className="flex items-center justify-between bg-zinc-900/40 p-3 rounded-2xl border border-zinc-800 shadow-lg relative z-[50]">
          <div className="flex items-center gap-1.5">
            ...
            <BotonTactil onAccion={() => setActiveHelp('header')} className="text-zinc-600 p-1"><Info size={14}/></BotonTactil>

            <BotonTactil onAccion={toggleLanguage} className="flex items-center gap-1 bg-zinc-800 border border-zinc-700 rounded-md px-1.5 py-1 xs:ml-2">
                <Globe size={12} className="text-zinc-400"/>
                <span className="text-[10px] font-black text-white uppercase">{lang}</span>
            </BotonTactil>
            <BotonTactil onAccion={() => setShowLogs(!showLogs)} className={`ml-1 flex items-center p-1.5 rounded-md border ${showLogs ? 'bg-zinc-700 border-zinc-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                <TerminalSquare size={14}/>
            </BotonTactil>
          </div>
          
          <div className="flex gap-1">
            <BotonTactil onAccion={toggleWakeLock} className={`p-1.5 rounded-lg border ${wakeLockActive ? 'bg-blue-600 text-white border-blue-600 shadow-[0_0_10px_rgba(37,99,235,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}><Eye size={18} /></BotonTactil>
            <BotonTactil onAccion={toggleSoundProfile} className={`p-1.5 rounded-lg border ${soundProfile !== 'muted' ? 'bg-amber-600 text-white border-amber-600 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>{soundProfile === 'siren' ? <Volume2 size={18}/> : soundProfile === 'radar' ? <Volume1 size={18}/> : <VolumeX size={18}/>}</BotonTactil>
            <BotonTactil onAccion={() => setShowSoundMenu(true)} className="p-1.5 bg-zinc-800 text-amber-500 border border-zinc-700 rounded-lg shadow-lg"><Music size={18}/></BotonTactil>
            <BotonTactil onAccion={() => { const next = !vibrateOn; setVibrateOn(next); if(!next) stopInfiniteAlarm(); if(syncRef.current) syncRef.current({ vibrateOn: next }); addLog(`Vibración de alertas: ${next ? 'ON' : 'OFF'}`, 'info'); }} className={`p-1.5 rounded-lg border ${vibrateOn ? 'bg-amber-500 text-black border-amber-500 shadow-lg shadow-amber-900/40' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}><VibrateIcon size={18}/></BotonTactil>
            
            <BotonTactil onAccion={toggleSystemNotifications} className={`p-1.5 rounded-lg border ${sysNotifOn ? 'bg-emerald-600 text-white border-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-zinc-800 text-zinc-500 border-zinc-700'}`}>
                <Smartphone size={18}/>
            </BotonTactil>
          </div>
        </div>
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-[32px] p-6 relative overflow-hidden shadow-2xl">
          ...
          <div className="pt-3 border-t border-zinc-800/50 flex justify-between items-end">
            <div className="flex items-center gap-1.5">
                ...
                <BotonTactil onAccion={() => setActiveHelp('time')} className="text-zinc-600 mb-1"><Info size={12}/></BotonTactil>
            </div>
          ...
          <div className="mt-5 border-t border-zinc-800/50 pt-4">
              <div className="flex justify-between items-center mb-3">
                  ...
                  <BotonTactil onAccion={syncWar} className="bg-zinc-800 text-zinc-400 border border-zinc-700 text-[9px] font-black py-1 px-3 rounded-full flex items-center gap-1"><RotateCcw size={10} /> {t('sync')}</BotonTactil>
              </div>
                      ...
                      if (a.custom) {
                          return (
                              <div key={a.id} className={`flex-[2] flex items-center justify-between rounded-xl border ${a.on ? 'bg-red-600 border-red-500' : 'bg-zinc-950 border-zinc-800'}`}>
                                  ...
                                  <BotonTactil onAccion={() => toggleAlarm(a.id)} className={`h-full px-2.5 rounded-r-xl border-l flex items-center justify-center block ${a.on ? 'border-red-700 bg-red-700' : 'border-zinc-800'}`}><div className={`w-2 h-2 rounded-full ${a.on ? 'bg-white shadow-[0_0_5px_white]' : 'bg-zinc-700'}`} /></BotonTactil>
                              </div>
                          );
                      }
                      return ( <BotonTactil key={a.id} onAccion={() => toggleAlarm(a.id)} className={`flex-1 rounded-xl text-xs font-black border block ${a.on ? 'bg-red-600 text-white border-red-500 shadow-[0_0_15px_rgba(220,38,38,0.3)]' : 'bg-zinc-950 text-zinc-500 border-zinc-800'}`}>{a.mins}M</BotonTactil> );
                  })}
              </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 relative">
            <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10">
                 <BotonTactil onAccion={() => setActiveHelp('creation')} className="bg-zinc-900 border border-zinc-700 text-zinc-500 p-1.5 rounded-full shadow-lg block"><Info size={14}/></BotonTactil>
            </div>
            
            <BotonTactil onAccion={() => { setShowCronoForm(!showCronoForm); setShowBoxForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 block ${showCronoForm ? 'bg-amber-500 text-black border-amber-500' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
                {t('addCrono')}
            </BotonTactil>
            <BotonTactil onAccion={() => { setShowBoxForm(!showBoxForm); setShowCronoForm(false); }} className={`py-3 rounded-2xl border font-black text-[10px] uppercase flex items-center justify-center gap-2 block ${showBoxForm ? 'bg-blue-600 text-white border-blue-600' : 'bg-zinc-900 text-zinc-400 border-zinc-800'}`}>
                {t('addSection')}
            </BotonTactil>
        </div>
        ...
        {showCronoForm && (
            <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-3 space-y-3 animate-in slide-in-from-top-2">
                <div className="flex justify-between items-center px-1"><span className="text-[10px] font-black uppercase text-zinc-500">{t('createCronoTitle')}</span><div className="flex gap-2"><BotonTactil onAccion={() => changeDraftCount(false, false)} className="p-1.5 bg-zinc-800 rounded-lg text-zinc-400 block"><Minus size={14}/></BotonTactil><BotonTactil onAccion={() => changeDraftCount(false, true)} className="p-1.5 bg-zinc-800 rounded-lg text-zinc-400 block"><Plus size={14}/></BotonTactil></div></div>
                ...
                <BotonTactil onAccion={addCrono} className="w-full bg-amber-600 text-white rounded-xl py-3 font-black text-[10px] uppercase shadow-lg text-center block">{t('createCronoBtn')}</BotonTactil>
            </div>
        )}
        ...
        {showBoxForm && (
            <div className="bg-zinc-900 border border-zinc-700 rounded-3xl p-3 space-y-3 animate-in slide-in-from-top-2">
                ...
                <div className="flex justify-between items-center px-1">
                    ...
                    <div className="flex gap-1.5">{COLORS.map(c => <BotonTactil key={c.hex} onAccion={() => setNewBoxColor(c.hex)} className={`w-5 h-5 rounded-full border-2 block ${newBoxColor === c.hex ? 'border-white scale-110' : 'border-transparent opacity-50'}`} style={{ backgroundColor: c.hex }} />)}</div>
                </div>
                <div className="flex items-center justify-between px-1">
                    ...
                    {includeInitialCrono && ( <div className="flex gap-2"><BotonTactil onAccion={() => changeDraftCount(true, false)} className="p-1 bg-zinc-800 rounded text-zinc-400 block"><Minus size={12}/></BotonTactil><BotonTactil onAccion={() => changeDraftCount(true, true)} className="p-1 bg-zinc-800 rounded text-zinc-400 block"><Plus size={12}/></BotonTactil></div> )}
                </div>
                ...
                <BotonTactil onAccion={addBox} className="w-full bg-blue-600 text-white rounded-xl py-3 font-black text-[10px] uppercase shadow-lg text-center block">{t('createSectionBtn')}</BotonTactil>
            </div>
        )}
                    ...
                    return (
                        <div key={box.id} className={`rounded-xl relative flex flex-col ${isThisBoxDragged ? 'opacity-30 scale-[0.98] border-dashed border-2 border-amber-500' : 'opacity-100 shadow-lg'}`}>
                            ...
                            >
                                ...
                                {editingBoxId === box.id ? (
                                    <div className="flex-1 flex flex-col p-3 bg-zinc-900/90 w-full animate-in fade-in pointer-events-auto">
                                        <div className="flex gap-2 items-center w-full mb-3"><input className="min-w-0 w-full bg-zinc-800 text-xs font-black p-2 rounded outline-none border border-blue-500 uppercase text-white shadow-inner" value={editBuf.label} onChange={e => setEditBuf({...editBuf, label: e.target.value})} autoFocus placeholder={t('sectionNamePlaceholder')}/><div className="flex gap-1 shrink-0">{COLORS.map(c => <BotonTactil key={c.hex} onAccion={()=>setNewBoxColor(c.hex)} className={`w-5 h-5 rounded-full block ${newBoxColor === c.hex ? 'border-2 border-white scale-110 shadow-lg' : 'opacity-40'}`} style={{backgroundColor:c.hex}}/>)}</div><div className="flex gap-1 shrink-0 border-l border-zinc-700 pl-2"><BotonTactil onAccion={() => { setEditingBoxId(null); setEditBoxDrafts([]); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm block"><X size={14}/></BotonTactil><BotonTactil onAccion={() => saveBoxEdit(box.id)} className="p-2 bg-blue-600 rounded-lg text-white shadow-md block"><Check size={14}/></BotonTactil></div></div>
                                        <div className="flex items-center justify-between px-1 py-2 border-t border-zinc-800/80"><span className="text-[10px] text-zinc-400 font-black uppercase tracking-widest flex items-center gap-1.5"><Plus size={10}/> {t('addCrono')}</span><div className="flex gap-1.5"><BotonTactil onAccion={() => handleEditBoxDraftCount(false)} className="p-1.5 bg-zinc-800 rounded-lg text-zinc-400 shadow-sm block"><Minus size={12}/></BotonTactil><BotonTactil onAccion={() => handleEditBoxDraftCount(true)} className="p-1.5 bg-blue-600/20 text-blue-400 rounded-lg shadow-sm block"><Plus size={12}/></BotonTactil></div></div>
                                        ...
                                        <div className="absolute right-3 flex items-center pointer-events-auto h-full">
                                            {isMenuOpen ? (
                                                <div className="flex items-center gap-1.5 animate-in slide-in-from-right-8 fade-in duration-200">
                                                    <BotonTactil onAccion={() => setConfirmBoxReset({id: box.id, name: box.name})} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm block" title="Reiniciar todos"><RotateCcw size={16}/></BotonTactil><BotonTactil onAccion={() => handleBoxPlayPause(box.id, !allPausedOrFinished)} className={`p-2 bg-zinc-800 border border-zinc-700 rounded-lg shadow-sm block ${!allPausedOrFinished ? 'text-zinc-400' : 'text-zinc-400'}`} title={!allPausedOrFinished ? 'Pausar todos' : 'Reanudar todos'}>{!allPausedOrFinished ? <Pause size={16}/> : <Play size={16}/>}</BotonTactil><div className="w-px h-6 bg-zinc-700 mx-1"></div><BotonTactil onAccion={() => { setEditingBoxId(box.id); setEditBuf({label: box.name}); setNewBoxColor(box.color); setEditBoxDrafts([]); setOpenBoxMenuId(null); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm block"><Edit2 size={16}/></BotonTactil><BotonTactil onAccion={async () => { const nb = boxes.filter(b => b.id !== box.id); let nr = rootOrder.filter(i => i.id !== box.id); const nt = tasks.map(t => { if (t.boxId === box.id) { nr.push({ id: t.id, type: 'task' }); return { ...t, boxId: null }; } return t; }); setBoxes(nb); setRootOrder(nr); setTasks(nt); if(syncRef.current) syncRef.current({boxes: nb, rootOrder: nr, tasks: nt}); }} className="p-2 bg-zinc-800 border border-zinc-700 rounded-lg text-zinc-400 shadow-sm block"><Trash2 size={16}/></BotonTactil><BotonTactil onAccion={() => setOpenBoxMenuId(null)} className="p-2 ml-1 text-zinc-500 rounded-full block"><X size={18}/></BotonTactil>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-1.5 animate-in fade-in"><BotonTactil onAccion={() => setOpenBoxMenuId(box.id)} className="p-2 text-zinc-500 rounded-lg block"><Settings size={20} /></BotonTactil><div className="w-px h-6 bg-zinc-800"></div><BotonTactil onAccion={() => toggleBoxCollapse(box.id)} className="p-2 text-zinc-400 rounded-lg block">{box.isCollapsed ? <ChevronDown size={22}/> : <ChevronUp size={22}/>}</BotonTactil></div>
                                            )}
                                        </div>
                                    </div>
                                )}
      ...
      {activeAlert && (
          <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md text-center pointer-events-auto">
            <div className={`w-full max-w-xs p-8 rounded-[40px] shadow-[0_0_50px_rgba(0,0,0,0.5)] animate-in zoom-in duration-300 border-4 flex flex-col relative ${currentAlertConf.bg} ${currentAlertConf.border}`}>
              ...
              <BotonTactil 
                onAccion={() => { stopInfiniteAlarm(); setActiveAlert(null); addLog("Intervención de usuario: Alerta silenciada.", "interaccion"); }} 
                className={`w-full py-4 rounded-2xl font-black text-lg uppercase shadow-xl tracking-widest border border-black/30 text-center block ${currentAlertConf.btnBg} ${currentAlertConf.btnText}`}
              >
